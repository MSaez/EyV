...
class OtController extends Controller
{
    ...
    public function actionGenot($id)
    {
        $model = $this->findModel($id);
        $modelsDesabolladura = $model->actividadDesabolladuras;
        $modelsPintura = $model->actividadPinturas;
        ...
        $content = $this->renderPartial('imprimir_ot',[
            'model' => $model,
            'modelsDesabolladura' => $modelsDesabolladura,
            'searchModelDesabolladura'=> $searchModelDesabolladura,
            'dataProviderDesabolladura'=> $dataProviderDesabolladura,
        ...
        $pdf = new Pdf([
            ...
            'destination' => Pdf::DEST_DOWNLOAD,
            // Nombre del archivo
            'filename' => 'Orden_de_Trabajo_folio_'.$model->OT_ID.'.pdf',
            // your html content input
            'content' => $content,  
            // format content from your own css file if needed or use the
            // enhanced bootstrap css built by Krajee for mPDF formatting 
            ...
            'cssInline' => '.kv-heading-1{font-size:18px}', 
            // set mPDF properties on the fly
            'options' => ['title' => 'Orden_de_Trabajo_folio_'.$model->OT_ID],
            // call mPDF methods on the fly
            'methods' => [ 
                //'SetHeader'=>['Krajee Report Header'], 
                //'SetFooter'=>['{PAGENO}'],
